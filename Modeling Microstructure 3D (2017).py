...
# creates a numerically simulated 3-D cube of a microstructure
...
def generate_canvas(sizeX, sizeY, sizeZ):
...
def generate_balls(canvas, volume_fraction, r_mean, r_std):
    ...
    sizeX = len(canvas)
    sizeY = len(canvas[0])
    sizeZ = len(canvas[0][0])
    canvas_volume = sizeX * sizeY * sizeZ
    ...
    for i in range(n_balls):
        ...
        while [nuc_x, nuc_y, nuc_z] in exclude:
            nuc_x = rand.randint(0, sizeX)
            nuc_y = rand.randint(0, sizeY)
            nuc_z = rand.randint(0, sizeZ)

        else:
            for x in range(sizeX):
                for y in range(sizeY):
                    for z in range(sizeZ):
                        if (x-nuc_x)**2 + (y-nuc_y)**2 + (z-nuc_z)**2 < r**2:
                            canvas[x][y][z] = 0

            for ex in range(sizeX):
                for ey in range(sizeY):
                    for ez in range(sizeZ):
                        if (ex-nuc_x)**2 + (ey-nuc_y)**2 + (ez-nuc_z)**2 < (2*r)**2:
                            exclude.append([ex, ey, ez])
...
def generate_file_fg(canvas, m):
    ...
    file_out = str("/Volumes/Aroha Whanau/Bergman Research/USWS 2017/Microstructure Models/Sn-4pct-Zn FG " + str(sizeX) +
                   "x" + str(sizeY) + "x" + str(sizeZ) + " x" + str(m) + ".txt")
    w = open(file_out, "w")

    for x in range(sizeX):
        for y in range(sizeY):
            for z in range(sizeZ):
                w.write(str(canvas[x][y][z]))
                w.write("   ")
            w.write("     ")
        w.write("\n\n\n")
...
def read_file(file):
    ...
    for line in f:
        ...
        for item in line_list:
            ...
            c[line_list.index(item)].append(item_list)
...
def generate_fg(x, y, z, m):
    # simulates a fine-grained microstructure
    canvas = generate_canvas(x, y, z)
...
# generate_fg(50, 50, 50, 40)


def generate_file_ds(canvas, m):
    # outputs a file containing the canvas and returns the file
    sizeX = len(canvas)
    sizeY = len(canvas[0])
    sizeZ = len(canvas[0][0])
...
def generate_ds(x, y, z, m):
    ...
    generate_rectangles(canvas, 0.545, sizeP, sizeG)
    # generate_hexagons(canvas, 0.545, sizeP, sizeG)

    file_in = generate_file_ds(canvas, m)
    # canvas_w = read_file(file_in)

    plt.matshow(canvas, cmap="gray")
    plt.show()


generate_ds(100, 100, 100, 40)
